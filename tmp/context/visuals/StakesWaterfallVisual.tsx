...
// Simplified data - no individual colors, derive from type
const waterfallData = [
...
export function StakesWaterfallVisual({ isVisible }: StakesWaterfallVisualProps) {
  ...
  useEffect(() => {
    ...
    // Animate each bar sequentially with longer, more dramatic delays
    ...
    }, 800) // Slower, more cinematic pacing

    // Show summary after all bars animate
  ...
  return (
    <div className="w-full max-w-xl mx-auto space-y-8">
          ...
          >
            ...
            <div className="flex justify-between items-baseline mb-3">
              <span className={`text-xs uppercase tracking-widest ${
                isLoss ? 'text-[var(--color-text-muted)]' : 'text-[var(--color-text-muted)]'
              }`}>
                {item.label}
              </span>
              <span 
      ...
      {/* Minimal summary - elegant inline text */}
      <div 
      ...
      >
        <div className="flex items-baseline justify-between">
          ...
          <span className="text-3xl font-light tabular-nums text-[var(--color-text-primary)]">
            {showSummary ? (
              <>
                -<AnimatedCounter value={16} prefix="$" suffix="M" decimals={0} duration={1500} />
              </>
            ) : (
              '-$0M'
            )}
