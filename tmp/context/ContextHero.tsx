...
export function ContextHero({ title, subtitle, onScrollDown }: ContextHeroProps) {
  const [isVisible, setIsVisible] = useState(false)

  useEffect(() => {
    // Trigger fade-in animation after mount - delayed to let background establish
    const timer = setTimeout(() => setIsVisible(true), 500)
    return () => clearTimeout(timer)
  }, [])

  return (
    <section className="context-section relative min-h-screen flex flex-col items-center justify-center px-6 overflow-hidden">
      
      {/* Layer 1: Single consolidated gradient orb */}
      ...
      <div 
        ...
        style={{
          ...
          top: '50%',
          transform: 'translate(-50%, -50%)',
          background: 'radial-gradient(ellipse at center, var(--color-accent-primary) 0%, transparent 60%)',
          opacity: 0.04,
          filter: 'blur(60px)',
      ...
      <div 
        ...
        style={{
          ...
          transition: 'opacity 3.5s cubic-bezier(0.25, 0.1, 0.25, 1), transform 5s cubic-bezier(0.16, 1, 0.3, 1)',
        }}
      >
        <h1 
          className="text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-bold text-[var(--color-text-heading)] leading-tight tracking-tight whitespace-pre-line"
        >
          {title}
        </h1>
        ...
        {subtitle && (
          <p 
            ...
            style={{
              transition: 'opacity 1.5s cubic-bezier(0.4, 0, 0.2, 1) 0.7s, transform 1.5s cubic-bezier(0.4, 0, 0.2, 1) 0.7s',
            }}
          >
            {subtitle}
          </p>
